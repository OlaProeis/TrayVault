...
- Fallback: Segoe UI when Roboto has no cmap entry (`GetGlyphIndicesW` + `GGI_MARK_UNAVAIL`) or rasterized ink is empty.
- Substitution: U+2010/U+2011/U+2012 (hyphen, non-breaking hyphen, figure dash) rasterize as ASCII `-` because bundled Roboto can expose a cmap slot that still draws a tofu box (en/em dashes U+2013+ are left as-is).
